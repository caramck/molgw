...
subroutine optical_spectrum(is_triplet_currently, basis, occupation, c_matrix, chi, xpy_matrix, xmy_matrix, eigenvalue, xi_eigenvalue)
  ...
  do t_jb_global=1, nexc
    ...
    if( is_triplet_currently ) then
      ...
      write(stdout,'(a)') ' Dot product:'
      write(stdout,'(es16.6)') DOT_PRODUCT(residue(:,t_jb_global),residue(:,t_jb_global))
      write(stdout,'(a,i4,a,es16.6)') ' Eigenvalue for t_jb_global=',t_jb_global,':',eigenvalue(t_jb_global)
