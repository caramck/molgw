...
!=====
 ...
 do iscf=1,nscf
   ...
   do ispin=1,nspin
     hamiltonian_test_vxc(:,:,ispin) = hamiltonian_test_vxc(:,:,ispin) - (hamiltonian_kinetic(:,:) + hamiltonian_nucleus(:,:) )
   enddo
...
subroutine print_exchange_expectations(basis,c_matrix,occupation,hamiltonian_exx_alpha,hamiltonian_exx_beta,vxc_ao)
 ...
 !=====
  ...
  !Read RESTART
  call clean_allocate('RESTART: C',c_matrix_restart,nbf,nstate,nspin)
  allocate(energy_restart(nstate,nspin))
  allocate(occupation_restart(nstate,nspin))
  allocate(h_ii(nstate,nspin))
  ...
  call matrix_ao_to_mo_diag(c_matrix_restart,RESHAPE(hamiltonian_exx_alpha,(/nbf,nbf,1/)),h_ii)
